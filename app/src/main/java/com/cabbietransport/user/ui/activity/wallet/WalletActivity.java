...
import static com.cabbietransport.user.ui.activity.payment.PaymentActivity.PICK_PAYMENT_METHOD;

public class WalletActivity extends BaseActivity implements WalletIView {

    private static final String WALLET_ACTIVITY_PATH = "com.cabbietransport.user.ui.activity.wallet.WalletActivity";
    ...
    public void initView() {
        ...
        if (!MvpApplication.isCard && !MvpApplication.isBraintree && !MvpApplication.isPaytm && !MvpApplication.isPayumoney && !MvpApplication.isTelr) {
    ...
    public void onSuccess(TelrResponse response) {
        try {
            ...
            String lastTransactionId = response.getTransactionId();

            TelrHelper.initializePayment(WalletActivity.this, amount.getText().toString(), WALLET_ACTIVITY_PATH, lastTransactionId);
...
//        Toast.makeText(this, wallet.getMessage(), Toast.LENGTH_SHORT).show();
    ...
    @Override
    public void onSuccess(TelrAddWallet wallet) {
        try {
            hideLoading();
        } catch (Exception e1) {
            e1.printStackTrace();
