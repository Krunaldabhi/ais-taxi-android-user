...
import com.cabbietransport.user.ui.fragment.cancel_invoice.CancelInvoiceFragment;
        ...
        switch (view.getId()) {
            ...
            case R.id.submit:
                ...
                if ((last_selected_location == cancelResponseList.size() - 1))
                    reason = cancelReason.getText().toString();
                else
                    reason = cancelResponseList.get(last_selected_location).getReason();
                SharedHelper.putKey(getContext(), TelrHelper.TELR_CANCEL_REASON_KEY, reason);
                ((MainActivity)baseActivity()).changeFragment(new CancelInvoiceFragment());
//                cancelRequest();
                break;
        }
    }
