...
import com.cabbietransport.user.data.network.model.Payment;
...
public class InvoiceFragment extends BaseFragment implements InvoiceIView {
    ...
    private static final String MAIN_ACTIVITY_PATH = "com.cabbietransport.user.ui.activity.main.MainActivity";
    ...
    private void initView(@NonNull Datum datum) {
        ...
        try {
            if (payment != null) {
                ...
                if(payment.getmUAESurcharges() > 0) {
                    surcharge.setText(String.format("%s %s",
                            getKey(Objects.requireNonNull(getContext()), "currency"),
                            getNewNumberFormat(payment.getmUAESurcharges())));
                } else llSurchageContainer.setVisibility(View.GONE);
...
//        tvChange.setVisibility((!MvpApplication.isCard && MvpApplication.isCash) ? View.GONE : View.VISIBLE);
    ...
    @Override
    public void onSuccess(TelrResponse response) {
        try {
            hideLoading();

            String lastTransacrionId = response.getTransactionId();
            SharedHelper.putKey(getContext(), TelrHelper.TELR_TIPS_KEY, String.valueOf(tips));
            SharedHelper.putKey(getContext(), TelrHelper.TELR_CANCELLED_TRANSACTION_KEY, false);
            TelrHelper.initializePayment(getContext(), String.valueOf(payment.getPayable() + tips), MAIN_ACTIVITY_PATH, lastTransacrionId);
        } catch (Exception e1) {
            e1.printStackTrace();
        }
    }
...
//                        tvChange.setVisibility(View.VISIBLE);
                    ...
                    if(data.getStringExtra("payment_mode").equals(Constants.PaymentMode.TELR)) {
        ...
        switch (paymentMode) {
            ...
            case Constants.PaymentMode.TELR:
                ...
                if (MvpApplication.RIDE_REQUEST.containsKey(CARD_LAST_FOUR))
                    text = getString(R.string.telr) + " - " + getString(R.string.card_, MvpApplication.RIDE_REQUEST.get("card_last_four"));
                tvPaymentMode.setText(text);
                break;
