package com.cabbietransport.user.ui.fragment.cancel_invoice;
...
//            initView(MvpApplication.DATUM);
    ...
    private void hideExtraInvoiceViews(@NonNull Datum datum) {
        ...
        fixed.setText(String.format("%s %s",
                ...
                getNewNumberFormat(20.00)));

        tax.setText(String.format("%s %s",
                getKey(Objects.requireNonNull(getContext()), "currency"),
                getNewNumberFormat(0.00)));
        ...
        initPaymentView(Constants.PaymentMode.TELR, "", false);
...
//        tvChange.setVisibility((!MvpApplication.isCard && MvpApplication.isCash) ? View.GONE : View.VISIBLE);
    ...
    public void onSuccess(TelrResponse response) {
        try {
            ...
            SharedHelper.putKey(getContext(), TelrHelper.TELR_CANCELLED_TRANSACTION_KEY, true);
            TelrHelper.initializePayment(getContext(), String.valueOf(20.00), MAIN_ACTIVITY_PATH, lastTransacrionId);
        } catch (Exception e1) {
            e1.printStackTrace();
        }
    }
